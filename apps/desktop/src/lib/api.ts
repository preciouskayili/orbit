...
import { workspaceComputers, projectComputers } from "./orbit-selectors";
...
export const orbitApi = {
  ...
  machines: async (projectId: string) => {
    requireProject(projectId);
    return projectComputers(getOrbitState(), projectId);
  },
  machine: async (machineId: string) => {
    const machine = workspaceComputers(getOrbitState()).find(
      (m) => m.id === machineId,
    );
    if (!machine) throw new Error("Computer not found.");
    return machine;
  },
  activity: async (projectId: string) => {
    requireProject(projectId);
    return getOrbitState().activity.filter((a) => a.projectId === projectId);
  },
  messages: async (projectId: string) => {
    requireProject(projectId);
    return getOrbitState().messages.filter((m) => m.projectId === projectId);
  },
