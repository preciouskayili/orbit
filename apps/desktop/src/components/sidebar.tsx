...
import { Bell, Buildings, CaretDown, CaretRight, Check, Folder, FolderOpen, Gear, MagnifyingGlass, Monitor, Robot, ChatCircleDots } from "@/components/ui/icons";
...
export function Sidebar() {
  ...
  return <aside className="flex min-h-0 w-72 shrink-0 flex-col bg-black/10">
    ...
    <div className="flex h-11 shrink-0 items-center px-5">
      <Link to="/new" onClick={() => orbitActions.newConversation()} className="text-lg font-medium text-zinc-100">Orbit</Link>
      <button aria-label="Search workspace (⌘K)" onClick={() => setSearchOpen(true)} className="ml-auto rounded-lg p-2 text-zinc-500 hover:bg-white/5"><MagnifyingGlass className="size-4" /></button>
    ...
    <nav aria-label="Main navigation" className="mt-2 shrink-0 space-y-0.5 px-4">
      {navigation.map(item => <Link key={item.path} to={item.path} onClick={() => { if (item.path === "/new") orbitActions.newConversation(); }} aria-current={location.pathname === item.path ? "page" : undefined} className={row + (location.pathname === item.path ? " bg-white/[0.065] text-zinc-100" : " text-zinc-400")}><item.icon className="size-3.5" />{item.label}</Link>)}
    </nav>
    <div className="sidebar-scroll mt-6 min-h-0 flex-1 overflow-y-auto"><div className="sidebar-scroll-content pb-4">
        ...
        {projects.map(project => { const expanded = !closed.has(project.id); const Icon = expanded ? FolderOpen : Folder;
          return <div key={project.id}>
            ...
            {expanded && <div className="mt-1 space-y-0.5">
              ...
              {tasks.filter(t => t.projectId === project.id).slice(0, 3).map(task => <Link key={task.id} to={"/sessions/" + task.id} onClick={() => orbitActions.openConversation(task.id)} className={row + " !pl-10 " + (location.pathname === "/sessions/" + task.id ? "bg-white/[0.075] text-zinc-200" : "text-zinc-500")}><span className="truncate">{task.title}</span></Link>)}
            </div>}
          </div>;
        })}
      ...
      {tasks.slice(0, 15).map(task => <Link key={task.id} to={"/sessions/" + task.id} onClick={() => orbitActions.openConversation(task.id)} className={row + " text-zinc-400"}><span className="min-w-0 flex-1 truncate">{task.title}</span>{task.status === "review" && <span className="size-2 rounded-full bg-sky-300" />}</Link>)}
      {!tasks.length && <p className="px-2.5 py-2 text-xs text-zinc-600">Your conversations will appear here.</p>}
    </div></div>
    ...
    <Dialog open={notifications} onOpenChange={setNotifications}><DialogContent className="p-6"><DialogTitle>Notifications</DialogTitle><DialogDescription className="mt-2">Agent results waiting for your review.</DialogDescription><div className="mt-5 space-y-2">{reviews.map(t => <Link key={t.id} to={"/sessions/" + t.id} onClick={() => { orbitActions.openConversation(t.id); setNotifications(false); }} className="block rounded-xl bg-white/5 p-3 text-sm text-zinc-300">{t.title}<span className="mt-1 block text-xs text-sky-300">Ready for review</span></Link>)}{!reviews.length && <p className="text-sm text-zinc-500">You're all caught up.</p>}</div></DialogContent></Dialog>
  </aside>;
}
