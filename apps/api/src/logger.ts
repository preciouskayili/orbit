...
export function withRequestId(id: string, fn: () => void) { requestContext.run(id, fn); }
...
export function log(level: Level, event: string, fields: Fields = {}) {
  ...
  const record = JSON.stringify({ time: new Date().toISOString(), level, event, requestId: requestContext.getStore(), ...fields });
  if (level === "error") console.error(record);
  else if (level === "warn") console.warn(record);
  else console.log(record);
}
export function errorFields(error: unknown): Fields {
  const value = error as { name?: string; code?: string; status?: number; statusCode?: number } | null;
  return {
