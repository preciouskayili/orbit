...
import { AgentRuns, OpenAIAgentModel } from "./agents/service.js";
...
const service = process.env.DAYTONA_API_KEY
  ? new DaytonaComputers(new Daytona({ requestTimeoutMs: 30_000 }), {
      workspaceId,
      instanceId,
      autoStopMinutes,
      vncPort,
...
const agents =
  service && process.env.OPENAI_API_KEY
    ? new AgentRuns(
        new OpenAIAgentModel(
